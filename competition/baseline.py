...
def get_feat2(df):
    ...
    # area_sum = df.groupby('小区名')['房屋面积'].sum().reset_index()
    # area_sum = ['小区名', '小区房屋总面积']
    # df = df.merge(area_sum, how='left', on='小区名')
...
def get_lgb_model(x_train, y_train):
    ...
    param_grid = {'learning_rate': [0.1],
                  "num_leaves": [800],
                  # "n_estimators": [1000, 1500, 2000]    #20000 1.8434
