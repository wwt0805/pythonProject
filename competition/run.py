...
test_data['距离'] = test_data['距离'].fillna(test_data["距离"].mean())
...
def get_xgb_model(x_train, y_train):
    estimator = xgb.XGBRegressor(max_depth=10,  # 构建树的深度，越大越容易过拟合
                                 n_estimators=1000,
                                 n_jobs=-1,  # 启动cpu所有核
...
def get_lgb_model(x_train, y_train):
    estimator = lgb.LGBMRegressor(num_leaves=800,
                                  n_estimators=1000,  # 1.95989
                                  # n_estimators=2000,  # 1.83479
                                  # n_estimators = 3000, # 1.84490
